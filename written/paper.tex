...
Let $I$ be the assignment of TA's to Labs. Let $L$ be the set of
labs/tutorials. Let $T$ be the set of all TA's.\\

$pr = (I, L, T)$\\

When is a branch solvable/unsolvable:\\

$Erw_{v,wt}((pr, ?),(pr, no))$ is never the case since this is an
optimization problem. As many contraints can be violated as necessary
to find a valid configuration.\\

$Erw_{v,wt}((pr, ?),(pr, no)) \iff L = \empty \land T \empty 
...
Let $A = (S, T)$ be our model where $T: S \times S$.s
