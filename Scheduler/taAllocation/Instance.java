...
public class Instance {
	...
	/*
	 * Generates a facts for the S_0 state of our set based search model. I was
	 * going to make a Fact class but a class called Solution already exists and
	 * will suffice.
	 */
	public Solution factFactory() {
		...
		Pair<TA, Lab> head = new Pair<TA, Lab>(TAs.remove(0), labs.remove(0));
		OTree<Pair<TA, Lab>> root = new OTree<Pair<TA, Lab>>(head);
		
		OTree<Pair<TA, Lab>> curr = root;
		for (Lab lab : labs) {
			for (TA ta : TAs) {
	...
	public boolean isValid() {
		...
		for (TA ta : TAs) {
			...
			if (ta.getLabs().size() > maxlabs)
				return false;

			// every TA is assigned at least MIN_LABS labs (if the TA *has* a
			// lab assignment)
			// FORALL ta:TA . lab-count(ta)/=0 -> lab-count(ta) >= MIN_LABS)
			if (!ta.getLabs().isEmpty() && ta.getLabs().size() < minlabs)
				return false;
		}
		...
		for (Lab lab : labcount.keySet()) {
			...
			// FORALL course:Course, lab:Lab | has-lab(course,?,lab). EXITS
			// ta:TA . instructs(ta,course,lab)
			if (labcount.get(lab) <= 0)
				return false;
		...
		for (TA ta : tacount.keySet()) {
			...
			// instructs(ta,c1,b1) /\ instructs(ta,c2,b2) . ~ EXISTS t1,t2 |
			// at(c1,b1,t1) /\ at(c2,b2,t2) . conflicts(t1,t2)
			if (tacount.get(ta) != 1)
				return false;
		}
		...
		// ((~ EXISTS c:Course, b:Lab | taking(ta,c,b) . EXISTS t1,t2 |
		// at(course,lab,t1) /\ at(c,b,t2)) . conflicts(t1,t2)))
		...
		for (Pair<TA, Lab> a : allocation) {
			...
			for (Timeslot ts : times.get(ta)) {
				if (ts.conflicts(lab.getTimeslot()))
					return false;
			}
			// check for conflicts between courses
			for (Lecture lec : ta.getTaking()) {
				if (lec.getTimeslot().conflicts(lab.getTimeslot()))
					return false;
			}
	...
	public int calcWert() {
		...
		for (TA ta : environment.getTAs()) {
			...
			if (current != former)
				wert -= 5;
			// No TA should teach more than 1 more lab then the TA who teaches
			// the least amount of Labs
			if ((current - least) >= 2)
				wert -= 25;
			...
			for (Lab lab : labs) {
				...
				if (distinct > 1)
					wert -= 10;
				// TA's should not have their labs in more then 2 different
				// courses
				if (counter > 1)
					wert -= 35;
				temp = lab;
