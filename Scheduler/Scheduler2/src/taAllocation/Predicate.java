...
public class Predicate {
	 ...
	 * malformed, throws a ParseException exception. Describes errors by
	 * printing to System.out.
	 * 
	 * @param s
	 *            a <code>String</code> from which to parse the predicate data.
	...
	/**
	 * Scans a parameter list or a list of set elements and returns a vector of
	 * pairs (type x value).
	 * 
	 * @param list
	 *            the input String.
	 * @param curInt
	 *            the index at which to start scanning the string.
	 * @param terminator
	 *            the character that will terminate the input.
	 * @param separator
	 *            the character that separates the input elements.
	 * @return a vector of (type x value) representing the list elements.
	 * @throws ParseException
	 *             if the input <code>list</code> is malformed.
	 */
	private Vector<Pair<ParamType, Object>> scanList(String list, Int curInt,
			char terminator, char separator) throws ParseException {
		...
		while ((paramEnd = scanFor(list, ++cur, delim)) != -1) {
			...
			Object obj = null;
			while (Character.isSpaceChar(list.charAt(cur)))
				cur++;
			switch (list.charAt(cur)) {
			case '{':
				obj = new TreeSet<Pair<ParamType, Object>>(scanList(list,
						curInt.setValue(cur), '}', ','));
				cur = curInt.getValue();
				t = ParamType.SET;
				// paramEnd might have been wrong if there was more than one
				// element in the set
			...
			case '"':
				...
				t = ParamType.STRING;
				// paramEnd might have been wrong if there were escape
			...
			default:
				...
				if (Character.isJavaIdentifierStart(c) || c == terminator
						|| c == separator) {
					obj = list.substring(cur, paramEnd);
					t = ParamType.STRING;
				} else
					throw new ParseException("unrecognized token", cur);
			}
			ret.add(new Pair<ParamType, Object>(t, obj));
			cur = paramEnd;
			if (list.charAt(paramEnd) == terminator)
				break;
		}
		if (paramEnd == -1)
			throw new ParseException("no closing terminator '" + terminator
					+ "' found", cur);
		curInt.setValue(cur);
		return ret;
	}
	 ...
	 * 
	 * @return the number of parameters of this predicate. Note that an empty
	 *         predcate (eg. "hot()" still has arity of 1 (a empty string
	 *         parameter).
	 */
	public int getArity() {
		return params.size();
	 ...
	 * . Note that <code>index</code> should be less than the value returned by
	 * <code>getArity()</code>. The first parameter is at index 0.
	 * 
	 * @param index
	 *            an index into the vector of parameters.
	...
	/**
	 * Returns the value of a string parameter at index <code>index</code>. If
	 * the index is out of range or the parameter is not of a string type,
	 * returns <code>null</code>.
	 * 
	 * @param index
	 *            the index of the parameter to evaluate
	 * @return a <code>String</code> representing the value of parameter at the
	 *         index, or null.
	 */
	...
	/**
	 * Returns the value of a set parameter at index <code>index</code> as a
	 * <code>TreeSet&lt;Pair&lt;ParamType,Object&lt;&lt;</code>. Thus, nested
	 * sets are allowed. If the index is out of range or the parameter is not of
	 * a set type, returns <code>null</code>.
	 * 
	 * @param index
	 *            the index of the parameter to evaluate
	 * @return a <code>TreeSet</code> representing the value of parameter at the
	 *         index, or null.
	 */
	 ...
	 * . If the index is out of range or the parameter is not of a Long type,
	 * returns <code>null</code>.
	 * 
	 * @param index
	 *            the index of the parameter to evaluate
	 * @return a <code>Long</code> representing the value of parameter at the
	 *         index, or null.
	 */
	...
	private String toString(Vector<Pair<ParamType, Object>> v) {
		...
		for (Iterator<Pair<ParamType, Object>> i = v.iterator(); i.hasNext();) {
			...
			case SET:
				s.append('{')
						.append(toString(new Vector<Pair<ParamType, Object>>(
								(TreeSet<Pair<ParamType, Object>>) p.getValue())))
						.append('}');
				break;
			case LONG:
	 ...
	 * of string, not <em>startAt</em>). It will <b>not</b> pay attention to the
	 * any target charater that is preceded by a backslash. Thus, scanFor() can
	 * be used in conjunction with escape() and unescape() to guarentee that an
	 * unknown string can be parsed. For example, you need to deliniate an
	 ...
	 * contain } characters. Save your string, s, as
	 * <code>s="{"+escape(s,"{}")+"}"
	 * </code> you can safely use scanFor(s,1,"{}") to find the original
	 * terminating } deliminator. The expression:
	 * 
	 * <pre>
	 * unescape(s.substring(scanFor(s, 0, &quot;{&quot;) + 1,
	 * 		scanFor(s, scanFor(s, 0, &quot;{&quot;), &quot;}&quot;)), &quot;{}&quot;)
	 * </pre>
	 * 
	...
	private static int scanFor(String in, int startAt, String chars) {
		...
		for (pos = startAt; pos < end; pos++) {
			...
			if (c == '\\')
				pos++;
			else if (chars.indexOf(in.charAt(pos)) >= 0)
				break;
		}
		if (pos == end)
			return -1;
	 ...
	 * {@link casa.util.CASAUtil#toQuotedString(String)}) returns a String
	 * without the enclosing quotes and with an escaped double-quotes restored
	 * (unescaped). Extra chars after the closing quote are ignored.
	 * 
	 * @param s
	 *            the quoted string to de-quote
	 * @param startAt
	 *            the position is <em>s</em> to start at
	 * @return an unquoted, unescaped string
	 * @throws ParseException
	 *             if <em>s</em> can't be interpreted as a quoted string (eg:
	 *             first non-white-space isn't '"') starting at <em>startAt</em>
	 * @see casa.util.CASAUtil#unescape(String, String)
	 */
	private static String fromQuotedString(String s, int startAt)
			throws ParseException {
	 ...
	 * previous call to escape() with the same <em>escChars</em>. For example,
	 * <code>
	 *  casautil.unescape("\(\(\)\)","()")  ==>  "(())"
	 * </code> unescape() always unescapes the backslash character, so
	 * <code>unescape(x,"")</code> is exactly the same as
	 * <code>unescape(x,"\\")</code>. <br>
	 ...
	 * the string (if there were some escaped charaters). This can be undone by
	...
	private static String unescape(String in, String escChars) {
		if (in == null)
			return null;
		if (in.length() == 0)
			return ""; // can't be any escapes in 1-char String, and we need to
						// guarentee one pass through the loop
		escChars = escChars.concat("\\");
		StringBuffer out = new StringBuffer(in.length());
		char c = ' ', next = in.charAt(0); // fix it so the last line will do
											// the right thing if we don't enter
											// the loop
		int end = in.length() - 1;
		int i;
		for (i = 0; i < end; i++) {
		...
		/*
		 * if (c == '\\') { if (escChars.indexOf(next) < 0) out.append(next); }
